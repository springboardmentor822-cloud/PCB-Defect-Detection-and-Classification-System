...
documents = [strip_html(d) for d in documents]
documents = [d for d in documents if len(d.strip()) > 20]  # remove empty after stripping
...
def build_stopwords():
    ...
    # 🔥 NEW: handle contractions + noisy words
    extra_words = {
        'dont', 'doesnt', 'didnt', 'cant', 'wont',
        'isnt', 'arent', 'wasnt', 'werent',
        'thats', 'youre', 'theyre', 'weve', 'ive',
        'im', 'us', 'get', 'also', 'one', 'two',
        'would', 'could', 'should'
    }

    return stop_words.union(custom_words).union(extra_words)
...
def preprocess_documents(docs):
    ...
    for text in docs:
        ...
        # 🔥 FIX: normalize contractions BEFORE removing symbols
...
# 🔥 IMPROVED FILTER (less aggressive)
valid_pairs = [
    (orig, proc)
    for orig, proc in zip(documents, processed_docs)
    if len(proc.split()) >= 3   # better than char length
]

if not valid_pairs:
    st.error("Not enough meaningful content after preprocessing. Try different data.")
    st.stop()
...
# Auto-generate topic labels from top keywords — same style as notebook manual labels
# e.g. keywords ['space','nasa','launch'] → "Space, Nasa & Launch"
def auto_label(keywords):
    top3 = [w.capitalize() for w in keywords[:3]]
    return top3[0] + ", " + top3[1] + " & " + top3[2] if len(top3) == 3 else ", ".join(top3)

topic_labels = {i + 1: auto_label(topic_keywords[i]) for i in range(num_topics)}

df_results = pd.DataFrame({
    "Document_No":    range(len(documents)),
    "Dominant_Topic": nmf_dominant_topic,
    "Topic_Weight":   nmf_topic_weight.round(4),
    "Topic_Label":    [topic_labels[t] for t in nmf_dominant_topic],
...
with tab1:
    ...
    topic_dist["Topic_Label"] = topic_dist["Topic"].apply(lambda x: topic_labels[x])
    topic_dist["Label"] = topic_dist["Topic_Label"]
...
with tab2:
    ...
    st.success(
        f"🏆 Dominant Topic: **{topic_labels[dominant_topic_id]}** (Topic {dominant_topic_id}) — "
    ...
    kw_df = pd.DataFrame({
        ...
        "Topic_Label": [topic_labels[i+1] for i in topic_keywords],
    ...
    for idx in range(n_chart_topics):
        ...
        ax.set_title(f"Topic {idx+1}: {topic_labels[idx+1]}", fontsize=8)
        plt.tight_layout()
        with cols[idx % 3]:
            st.pyplot(fig_w)
        plt.close(fig_w)
...
with tab4:
    ...
    for i, (topic_id, words) in enumerate(list(topic_keywords.items())[:n_wc_topics]):
        ...
        ax_t.axis('off')
        ax_t.set_title(f"Topic {topic_id+1}: {topic_labels[topic_id+1]}", fontsize=8)
        with cols[i % 3]:
            st.pyplot(fig_t)
        plt.close(fig_t)
